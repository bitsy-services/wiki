...
1. [Single-tick liquidity](/wiki/defi/uniswap/single-tick-liquidity) — the entire supply seated in one [tick](/wiki/defi/uniswap/ticks) makes price a near-fixed rate inside a 1-bp corridor.
2. [Liquidity floor](/wiki/defi/liquidity-floor) — no liquidity below par means sells cannot push price under 1:1.
3. [Oracle-free pricing](/wiki/defi/oracle-free-pricing) — the corridor is a theorem about the position, not a feed; nothing to manipulate or stall.
4. [Locked liquidity](/wiki/defi/locked-liquidity) — the position has no decrease path, so the backing can never be unwound.
5. [Full-reserve backing](/wiki/defi/full-reserve-backing) — the reserve is the real originals accumulated 1:1 as buyers swap in.
6. [Permissionless token factory](/wiki/defi/permissionless-token-factory) — anyone can instantiate one for any ERC-20, no gate.
...
- **Currency boards and bearer instruments** — the traditional-finance economic analog: full reserve, fixed rate, no discretion; the issuer earns the [float](/wiki/defi/full-reserve-backing), not the principal — exactly a traveler's-cheque arrangement.
