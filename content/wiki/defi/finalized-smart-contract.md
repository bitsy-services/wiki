...
A finalized smart contract is one that has been deployed to a [blockchain](/wiki/defi/blockchain) with no mechanism for anyone -- including its original developer -- to modify its code or behavior. Once finalized, the contract runs exactly as written for as long as the blockchain exists.

This is the strongest form of the immutability property that makes [smart contracts](/wiki/defi/smart-contract) useful in the first place. A finalized contract cannot be censored, paused, upgraded, or tampered with by governments, corporations, or the developers who wrote it.

## What finalization means in practice

A smart contract on [Ethereum](/wiki/defi/ethereum/) is finalized when all of the following are true:
...
A finalized contract cannot be forced to comply with a court order, sanctions list, or government directive. This was a central issue in the Tornado Cash case -- the protocol's [smart contracts](/wiki/defi/smart-contract) continued to function on-chain even after OFAC sanctions, because they were finalized and no one had the ability to modify them. See [DeFi US regulatory restrictions](/wiki/defi/defi-us-regulatory-restrictions) for more on the legal implications.

### Composability guarantees

Other protocols that integrate with a finalized contract can rely on its behavior never changing. This is critical for [DeFi](/wiki/defi/dex) composability -- a lending protocol that accepts a certain token as collateral needs to trust that the token contract's transfer logic will not be altered.
...
- **Token contracts** -- [ERC-20](/wiki/defi/ethereum/erc-20) tokens with straightforward transfer logic.
