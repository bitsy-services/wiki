...
A cash-backed synthetic option (CBSO) is a fully collateralized, on-chain derivative that replicates the payoff of a traditional [option spread](/wiki/defi/vertical-spread) without requiring an actual underlying asset to change hands. The "cash-backed" part means every position is backed one-to-one by a settlement currency locked in a [smart contract](/wiki/defi/smart-contract) -- there are no margin calls and no liquidation risk. The "synthetic" part means the contract derives its value from an external price feed rather than from custody of the underlying asset.

[Bull bets](/wiki/defi/bull-bet) and [bear bets](/wiki/defi/bear-bet) are specific implementations of CBSOs, packaging [bull call spreads](/wiki/defi/bull-call-spread) and [bear put spreads](/wiki/defi/bear-put-spread) respectively into single mintable tokens.
...
- **Risk-defined by construction.** Every CBSO is a [risk-defined strategy](/wiki/defi/risk-defined-strategy) with capped profit and capped loss, structurally identical to a [vertical spread](/wiki/defi/vertical-spread).
...
A *minter* creates CBSOs by depositing settlement currency (such as USDC or ETH) into the CBSO [smart contract](/wiki/defi/smart-contract). The deposited amount equals the maximum payout of the option -- the width between the two [strike prices](/wiki/defi/strike-price). In return, the minter receives [ERC-20](/wiki/defi/ethereum/erc-20)-compliant tokens representing the option position.
...
At expiration, the smart contract settles each CBSO based on the final asset price reported by a decentralized [oracle](/wiki/defi/oracle-node). The payout is calculated from the relationship between the [strike price](/wiki/defi/strike-price) and the settlement price, capped at the locked collateral. Any remaining collateral is returned to the minter.
...
[Bull bets](/wiki/defi/bull-bet) and [bear bets](/wiki/defi/bear-bet) are the primary user-facing CBSO products:

| Product | Underlying strategy | Profits when |
|---------|-------------------|--------------|
| Bull bet | [Bull call spread](/wiki/defi/bull-call-spread) | Asset price rises above the breakeven |
| Bear bet | [Bear put spread](/wiki/defi/bear-put-spread) | Asset price falls below the breakeven |

By combining bull bets and bear bets at different strikes, traders can construct more complex positions -- iron condors, butterfly spreads, and other multi-leg strategies -- entirely on-chain. See [emulating option strategies](/wiki/defi/emulating-option-strategies) for details.

## Technical Design

The smart contract architecture behind CBSOs -- including the Option Factory, Option Settler, Asset Price Oracle, and [Fee Box](/wiki/defi/fee-box) -- is covered in [CBSO Design](/wiki/defi/cash-backed-synthetic-options-design).
