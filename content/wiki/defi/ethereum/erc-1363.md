...
The standard [ERC-20](/wiki/defi/ethereum/erc-20) workflow for interacting with a [smart contract](/wiki/defi/smart-contract) looks like this:
...
A [staking](/wiki/defi/staking) contract using ERC-1363:
...
| [ERC-677](/wiki/defi/ethereum/erc-677) | `transferAndCall` only | `onTokenTransfer` | Chainlink LINK |
...
- **Not widely adopted yet** -- most existing [DeFi](/wiki/defi/dapp) contracts expect the traditional approve-then-call pattern. A token being ERC-1363 doesn't help unless the receiving protocol supports the callback.
