...
The constant mean formula is the pricing invariant behind Balancer's weighted pools. Where the [constant product formula](/wiki/defi/constant-product-formula) constrains two-asset pools to a 50/50 split, the constant mean formula generalises to **any number of assets** with **arbitrary weight ratios** -- 80/20, 60/20/20, or any combination that sums to 100%.

This makes it possible to build [liquidity pools](/wiki/defi/liquidity-pool) that behave like self-rebalancing index funds: LPs choose the asset mix they want exposure to, and the [AMM](/wiki/defi/amm) maintains those proportions through trading fees rather than manual rebalancing.
...
In a standard 50/50 [Uniswap](/wiki/defi/uniswap)-style pool, an LP's position rebalances aggressively as prices move -- selling the appreciating token and buying the depreciating one. This is the mechanism behind [impermanent loss](/wiki/defi/impermanent-loss).
...
Balancer is the primary [DEX](/wiki/defi/dex) built on the constant mean formula. Its pool types include:
