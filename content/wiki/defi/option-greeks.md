...
The **option greeks** are a set of sensitivity measures that describe how an option's price responds to changes in underlying variables -- spot price, time, [volatility](/wiki/defi/volatility), and interest rates. They are the primary tools for understanding and managing risk in options positions.
...
In crypto markets, where [volatility](/wiki/defi/volatility) can shift dramatically, vega exposure often dominates other greeks. A position can be delta-neutral but still suffer large losses from an IV contraction.
...
Understanding greeks is especially relevant for DeFi liquidity providers. Depositing into an options [liquidity pool](/wiki/defi/liquidity-pool) means implicitly taking the other side of user trades, which creates greek exposures that the pool manages through hedging or pricing adjustments.
