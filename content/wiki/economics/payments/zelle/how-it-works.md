...
Zelle does not move money. It is a directory and a messaging network. The money moves on rails that already existed, later, and in a different shape from what the user saw.

The speed, the irrevocability, and the absence of any Zelle balance or account to look at all follow from that.

## What happens when you press send

You open your bank's app, choose Zelle, and enter a mobile number, email address, or [tag](/wiki/economics/payments/zelle/tag) — an [alias](/wiki/economics/payments/zelle/alias) — plus an amount. What follows takes seconds and involves four parties.
...
It costs one thing. A push payment is complete the moment it is pushed. There is no clearing window in which anyone reconsiders, and consequently **no consumer-facing recall**. If the recipient is enrolled, the payment is done. Your bank can ask the receiving bank to return the funds as a courtesy; the receiving bank has no obligation to comply, and if the recipient has already moved the money, nothing to return. Where that loss falls is the subject of [fraud and liability](/wiki/economics/payments/zelle/fraud-and-liability).

Payments to an alias that is *not* yet enrolled behave differently, and are the one case where a sender gets a second chance — see [the unenrolled recipient](/wiki/economics/payments/zelle/alias#the-unenrolled-recipient).
...
Early Warning charges participating institutions to be on the network. Consumers pay nothing, and there is no interchange — which was the objective, since the alternative the banks were defending against was a fintech capturing both the float and the fee.
...
- **2025** — The standalone Zelle app is retired. Around two percent of volume ran through it, and its existence had become a liability: an app that anyone could enroll in was the natural entry point for the [enrollment attacks](/wiki/economics/payments/zelle/alias#enrollment-hijacking) the network wanted to eliminate. Zelle now exists only inside the app of a bank that has already identified you.

Retiring the app narrowed the network to its security model, which is neither cryptographic nor procedural: every participant on both ends has been through [know your customer](/wiki/economics/regulation/know-your-customer) at a regulated institution. Nothing else holds it up.
...
The limit is the *only* risk control the sending bank retains after the payment leaves, so it is set per-customer by the institution that carries the loss. Anyone building on Zelle has to treat the limit as a property of the individual sender rather than of the network, and discover it empirically.
