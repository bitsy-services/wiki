...
- **Absence floor.** There is simply no order book or [AMM](/wiki/economics/defi/amm) depth below the floor price. There is nothing to sell *into*. Nothing has to be spent to hold the line because holding the line is the default state.
...
On a V3/V4 pool, price moves by crossing [ticks](/wiki/economics/defi/uniswap/ticks). If no position covers any tick below some boundary `T`, then [V4's swap math](/wiki/economics/defi/uniswap) cannot push price below `T` — there is no liquidity for a seller to consume there, so the swap reverts or partially fills at `T`.

The clean construction:

1. Seat the entire token supply as a [single-tick position](/wiki/economics/defi/uniswap/single-tick-liquidity) at the floor tick, single-sided.
...
- **[Locked liquidity](/wiki/economics/defi/locked-liquidity).** The seeded position cannot be moved down or withdrawn and re-seated lower.
...
- **It is only a floor.** Absence below a tick says nothing about the ceiling. Bounding price on *both* sides requires also capping the upside — e.g. a [single-tick position](/wiki/economics/defi/uniswap/single-tick-liquidity) that holds the entire supply, so there is nothing to buy past the top of the tick either.
...
- **A par token combines this with other techniques.** On its own a liquidity floor just prevents downside. See [par token](/wiki/economics/defi/par-token) for the floor used as one half of a hard two-sided peg.
