...
[Oasis](https://oasis.net/) **Sapphire** is the first and, for now, the only general-purpose **confidential [EVM](/wiki/economics/defi/ethereum/)** — a fully Ethereum-compatible chain where [smart contract](/wiki/economics/defi/smart-contract) state and transaction calldata are *encrypted by default*. Ordinary [Solidity](/wiki/economics/defi/solidity/) compiles and runs unchanged, but the contract's storage, the arguments you send it, and the values it returns are sealed inside a hardware [trusted execution environment](/wiki/economics/defi/tee) (TEE) and never exposed to validators, RPC providers, or block explorers. Sapphire is a *ParaTime* (a runtime) on the Oasis Network; its gas token is **ROSE**.
...
- **Sealed-bid auctions and private order flow** — bids stay hidden until settlement, which structurally removes the front-running and [maximal extractable value](/wiki/economics/defi/maximal-extractable-value) that plague transparent [DEXes](/wiki/economics/defi/dex). A searcher cannot front-run an order it cannot read.
- **Private voting and [DAO](/wiki/economics/defi/dao) governance** — ballots that aren't visible mid-vote prevent vote-buying and bandwagoning.
