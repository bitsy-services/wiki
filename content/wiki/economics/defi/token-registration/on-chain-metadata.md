...
MetaMask recommends this method to token developers. Its `contract-metadata` repository, the pull-request route for getting an icon into the wallet, calls itself "effectively frozen" and points here — though it still merges submissions slowly, as [registering on MetaMask](/wiki/economics/defi/token-registration/metamask) describes. This call is the only MetaMask route that works on the day the token is deployed.
