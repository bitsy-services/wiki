...
A smart contract on [Ethereum](/wiki/economics/defi/ethereum/) is finalized when all of the following are true:
...
- **No self-destruct** -- the contract cannot be destroyed by its deployer (note: the `SELFDESTRUCT` opcode was deprecated in [EIP](/wiki/economics/defi/ethereum/eip)-6780, but contracts deployed before the change may still have it).
...
Other protocols that integrate with a finalized contract can rely on its behavior never changing. [DeFi](/wiki/economics/defi/dex) composability rests on this: a lending protocol that accepts a token as collateral is relying on that token's transfer logic never changing underneath it.
...
- **Token contracts** -- [ERC-20](/wiki/economics/defi/ethereum/erc-20) tokens with straightforward transfer logic.
