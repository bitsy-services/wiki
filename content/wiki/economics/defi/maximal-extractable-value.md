...
It is charged against every [DEX](/wiki/economics/defi/dex) trade, every lending-protocol liquidation, and every on-chain auction, and it surfaces downstream as reverted transactions, gas spikes, and protocol designs built specifically to deny it.

## Why transaction ordering creates value

On an [AMM](/wiki/economics/defi/amm) like [Uniswap](/wiki/economics/defi/uniswap), a large swap moves the price. Anyone who can insert a transaction *before* that swap — buying the token cheap — and another *after* — selling it at the new, higher price — captures risk-free profit. The original trader gets worse execution. This is a **sandwich attack**.
...
A searcher adds concentrated [liquidity](/wiki/economics/defi/liquidity-pool) to a pool in the block just before a large swap and removes it immediately after, capturing a disproportionate share of fees without taking on price risk.

## The MEV supply chain

On [Ethereum](/wiki/economics/defi/ethereum/) post-Merge, MEV extraction is split into specialised roles:
