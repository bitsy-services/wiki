...
Oracle-free pricing means a contract derives the price of an asset entirely from on-chain liquidity *geometry* — the shape and position of an [AMM](/wiki/economics/defi/amm) position — rather than reading an external price feed. There is no number to fetch and trust; the position **is** the price.

## What an oracle costs

A price [oracle](/wiki/economics/defi/oracle-node) imports an external fact onto the chain: "ETH is worth 3,000 USDC." Anything that depends on that import inherits its failure modes:
...
A [single-tick liquidity](/wiki/economics/defi/uniswap/single-tick-liquidity) position pins price to a 1-bp band by construction. The contract does not *read* that price — it *imposes* it, by being the only place a trade can happen and by occupying exactly one [tick](/wiki/economics/defi/uniswap/ticks). The relationship "1 issued token ≈ 1 reference token" is true not because a feed says so but because the swap math cannot produce any other answer within the seeded tick, and there is no liquidity outside it.
