...
title: "Sapphire"
weight: 40
aliases: ["/wiki/economics/defi/sapphire/"]
---

[Oasis](https://oasis.net/) **Sapphire** is the first and, for now, the only general-purpose **confidential [EVM](/wiki/economics/defi/chains/ethereum/)** — a fully Ethereum-compatible chain where [smart contract](/wiki/economics/defi/smart-contract) state and transaction calldata are *encrypted by default*. Ordinary [Solidity](/wiki/economics/defi/development/solidity/) compiles and runs unchanged, but the contract's storage, the arguments you send it, and the values it returns are sealed inside a hardware [trusted execution environment](/wiki/economics/defi/smart-contract/tee) (TEE) and never exposed to validators, RPC providers, or block explorers. Sapphire is a *ParaTime* (a runtime) on the Oasis Network; its gas token is **ROSE**.

## What encrypted state makes possible
...
- **Sealed-bid auctions and private order flow** — bids stay hidden until settlement, which structurally removes the front-running and [maximal extractable value](/wiki/economics/defi/markets/maximal-extractable-value) that plague transparent [DEXes](/wiki/economics/defi/markets/dex). A searcher cannot front-run an order it cannot read.
- **Private voting and [DAO](/wiki/economics/defi/smart-contract/dao) governance** — ballots that aren't visible mid-vote prevent vote-buying and bandwagoning.
...
On Ethereum, "on-chain" means "public." On Sapphire it can mean "private but verifiable."

## How Confidentiality Works

Sapphire executes contracts inside a [TEE](/wiki/economics/defi/smart-contract/tee) — historically Intel SGX (Software Guard Extensions) enclaves. The CPU encrypts enclave memory in hardware, so the machine operator running the node cannot read the contract's working state or storage even with full control of the host. Three things are protected:
...
- **Composability friction.** Sapphire is its own L1; it does not share liquidity or synchronous calls with Ethereum or other chains. Cross-chain interaction goes through bridges or the **Oasis Privacy Layer (OPL)**, which lets a contract on a transparent chain make confidential calls into Sapphire. It works, and it is another moving part with its own latency and trust surface.
...
- Intel TDX (Trust Domain Extensions) confidential VMs are easier to provision and audit than SGX application enclaves, which broadens the hardware base a confidential workload can run on and loosens Sapphire's dependence on one enclave design.
