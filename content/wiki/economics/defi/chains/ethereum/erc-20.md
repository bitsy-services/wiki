...
ERC-20 — the twentieth [Ethereum Request for Comment](/wiki/economics/defi/chains/ethereum/eip) — is the standard interface for fungible tokens on [Ethereum](/wiki/economics/defi/chains/ethereum/). Proposed by Fabian Vogelsteller in 2015 ([EIP](/wiki/economics/defi/chains/ethereum/eip)-20), it defines six functions and two events that every compliant token must implement. Wallets, [DEXs](/wiki/economics/defi/markets/dex), lending protocols, and aggregators all speak ERC-20, so a token that implements the interface is tradeable and usable as collateral on the day it deploys, without asking any of them to add support for it.
...
The two-step `approve` + `transferFrom` pattern is the standard way for a [smart contract](/wiki/economics/defi/smart-contract) to spend tokens on behalf of a user. The user first approves a spending allowance, then the contract calls `transferFrom` to pull the tokens.
...
Tether's `transfer` and `approve` functions return nothing. A [Solidity](/wiki/economics/defi/development/solidity/) contract that checks the return value with `require(token.transfer(...))` will revert when interacting with USDT, because the [EVM](/wiki/economics/defi/chains/ethereum#the-ethereum-virtual-machine-evm) interprets the missing return data as a failed `bool` decode.
...
The interface carries `name`, `symbol` and `decimals` and nothing else about the token's identity. There is no icon, no website, no description, and no issuer field, so every logo a wallet or explorer displays comes from an off-chain database that somebody had to submit the token to. [Token registration](/wiki/economics/defi/token-registration) covers where those databases are and what each one asks for. [ERC-1046](/wiki/economics/defi/token-registration/on-chain-metadata) adds an optional `tokenURI` that puts the metadata document back on the contract, at the cost of near-zero adoption among the things that would read it.
