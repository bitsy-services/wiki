...
IOTA [EVM](/wiki/economics/defi/chains/ethereum#the-ethereum-virtual-machine-evm) is IOTA's fully [Ethereum](/wiki/economics/defi/chains/ethereum/)-compatible smart-contract chain. It is the practical entry point for [DeFi](/wiki/economics/defi/markets/dex) on IOTA: existing [Solidity](/wiki/economics/defi/development/solidity/) contracts, [ERC-20](/wiki/economics/defi/chains/ethereum/erc-20) tooling, MetaMask, Hardhat, and Foundry all work unmodified, with configuration changes only. It launched on mainnet in 2024 and continues to run as a Layer 2 alongside the [Move](/wiki/economics/defi/chains/iota/iota-rebased) Layer 1 introduced by the [Rebased](/wiki/economics/defi/chains/iota/iota-rebased) upgrade.

## Architecture

IOTA EVM is one chain produced by **IOTA Smart Contracts (ISC)**, the framework that runs sandboxed contract chains and anchors their state to IOTA Layer 1. From a developer's point of view it is an ordinary EVM chain; the ISC layer underneath handles settlement to L1 and asset movement. The native [IOTA token](/wiki/economics/defi/blockchain/cryptocurrency) is the gas currency, and a bridge moves value between L1 native assets and L2 EVM balances.

IOTA EVM is *not* the Move L1 environment. Building L1-native applications means writing Move modules against the object ledger; IOTA EVM is the compatibility surface for porting Ethereum-ecosystem code. The Foundation has stated it intends to integrate EVM execution into Layer 1 directly, but as of mid-2026 that is still on the roadmap — the L2 chain described here remains the way to run Solidity on IOTA.
