...
Before anything touches a real network, Foundry runs the entire script in a local [EVM](/wiki/economics/defi/chains/ethereum#the-ethereum-virtual-machine-evm). This simulation executes in a single context, so `token.initialize(...)` can reference the address returned by `new MyToken()` even though they will eventually be separate transactions. If any statement reverts during simulation, the script aborts and nothing is broadcast.

### Phase 2 — Sequential broadcast

After simulation succeeds, Foundry replays the recorded state-changing operations as individual transactions, each with an incrementing [nonce](/wiki/economics/defi/blockchain/nonce). The transactions are submitted in the order they appeared in the script.
...
This is not a hypothetical — it comes up in practice on busy networks, especially with scripts that interact with shared protocol state ([AMM](/wiki/economics/defi/markets/amm) pools, governance contracts, registries).
