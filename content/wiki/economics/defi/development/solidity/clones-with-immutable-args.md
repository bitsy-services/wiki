...
[Minimal proxies](https://eips.ethereum.org/EIPS/eip-1167) ([EIP](/wiki/economics/defi/chains/ethereum/eip)-1167 clones) are the cheapest way to deploy many instances of the same contract. Each clone delegates every call to a shared **implementation** contract, so you only pay for ~45 bytes of creation code instead of re-deploying the full bytecode.
