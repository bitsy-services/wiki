...
ERC-1363 is an [ERC-20](/wiki/economics/defi/ethereum/erc-20) extension that adds callback hooks to token transfers and approvals. It removes ERC-20's two-step problem: `approve` + `transferFrom` costs two transactions and leaves an allowance standing between them, while a plain `transfer` to a contract that was not expecting it leaves the tokens sitting there with nothing to trigger the deposit.

With ERC-1363, a token holder can transfer tokens *and* notify the recipient in a single transaction. The standard was formalized as [EIP](/wiki/economics/defi/ethereum/eip)-1363 and has an [OpenZeppelin implementation](https://docs.openzeppelin.com/contracts/5.x/api/token/erc20#ERC1363).

## The approve-and-transfer problem

The standard [ERC-20](/wiki/economics/defi/ethereum/erc-20) workflow for interacting with a [smart contract](/wiki/economics/defi/smart-contract) looks like this:
...
A [staking](/wiki/economics/defi/staking) contract using ERC-1363:
...
| [ERC-677](/wiki/economics/defi/ethereum/erc-677) | `transferAndCall` only | `onTokenTransfer` | Chainlink LINK |
...
- **Not widely adopted yet** -- most existing [DeFi](/wiki/economics/defi/dapp) contracts expect the traditional approve-then-call pattern. A token being ERC-1363 doesn't help unless the receiving protocol supports the callback.
