...
ERC-20 — the twentieth [Ethereum Request for Comment](/wiki/economics/defi/ethereum/eip) — is the standard interface for fungible tokens on [Ethereum](/wiki/economics/defi/ethereum/). Proposed by Fabian Vogelsteller in 2015 ([EIP](/wiki/economics/defi/ethereum/eip)-20), it defines six functions and two events that every compliant token must implement. Wallets, [DEXs](/wiki/economics/defi/dex), lending protocols, and aggregators all speak ERC-20, so a token that implements the interface is tradeable and usable as collateral on the day it deploys, without asking any of them to add support for it.
...
Tether's `transfer` and `approve` functions return nothing. A [Solidity](/wiki/economics/defi/solidity/) contract that checks the return value with `require(token.transfer(...))` will revert when interacting with USDT, because the [EVM](/wiki/economics/defi/ethereum#the-ethereum-virtual-machine-evm) interprets the missing return data as a failed `bool` decode.
