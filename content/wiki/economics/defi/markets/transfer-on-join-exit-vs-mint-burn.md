...
weight: 130
aliases: ["/wiki/economics/defi/transfer-on-join-exit-vs-mint-burn/"]
---

When a user deposits assets into a [liquidity pool](/wiki/economics/defi/markets/liquidity-pool) or vault, the protocol needs a way to track their share. Two design patterns dominate: **transfer on join/exit**, which moves pre-existing tokens to represent membership, and **mint/burn**, which creates and destroys share tokens on the fly. The choice between them shapes a protocol's accounting model, composability, and gas profile.
...
- **Lower gas on entry/exit.** A storage write to an internal mapping is cheaper than minting a new [ERC-20](/wiki/economics/defi/chains/ethereum/erc-20) token (which requires updating `totalSupply`, the recipient's balance, and emitting a `Transfer` event).
- **No composability.** Because the share is an internal ledger entry rather than a transferable token, it cannot be used as collateral, traded on a [DEX](/wiki/economics/defi/markets/dex), or plugged into other protocols. The user's position is locked inside the pool.
...
This pattern appears in simple single-asset [staking](/wiki/economics/defi/markets/staking) contracts and some early yield-farming designs where composability was not a priority.
...
Most modern [AMMs](/wiki/economics/defi/markets/amm) -- including [Uniswap](/wiki/economics/defi/markets/uniswap) v2/v3 [LP](/wiki/economics/defi/markets/liquidity-pool) tokens and Balancer pool tokens -- use mint/burn. Yield aggregators and lending vaults almost universally follow this pattern.
