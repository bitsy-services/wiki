...
weight: 70
aliases: ["/wiki/economics/defi/virtual-reserves/"]
---

Virtual reserves are a mathematical abstraction used by [AMMs](/wiki/economics/defi/markets/amm) to make a limited amount of real capital behave like a much larger [liquidity pool](/wiki/economics/defi/markets/liquidity-pool). The idea is central to [Uniswap](/wiki/economics/defi/markets/uniswap) V3's concentrated liquidity and appears in various forms across other protocols.

## The problem virtual reserves solve

In a standard [constant product](/wiki/economics/defi/markets/constant-product-formula) pool, liquidity is spread across every price from zero to infinity. If a pool holds 10 ETH and 25,000 USDC, most of that capital sits at prices far from the current market and never facilitates a trade. The capital is real but idle.

Concentrated liquidity fixes this by letting [LPs](/wiki/economics/defi/markets/liquidity-pool) deploy capital within a chosen price range, which breaks the basic `x * y = k` formula: the actual token balances can no longer be fed to it directly, because one or the other reaches zero at the edge of the range. Virtual reserves are the bridge between a range-bound position and the constant product math.
