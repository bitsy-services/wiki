...
Against the on-chain flags, query GoPlus, honeypot.is and RugCheck directly — the recipes are on [designing and self-checking](/wiki/economics/defi/token-false-alarms/designing-and-self-checking#check-yourself).
...
The unbalanced bracket is Etherscan's. For a contract, sign from the creating address and adapt the wording to "I, the creator of the contract address". The neighbouring process for *claiming* a contract in order to update its token info documents the awkward cases that name-tag removal does not: a multisig signer signs a variant binding the claim to an Etherscan username, with a warning not to publish that signed message since it reveals the username, and the lost-key and community-takeover branches end at "Our team will guide you through the rest of the process". A contract created by a factory has a contract, not a key, as its creator in the sense Etherscan's documentation uses; [registering on Etherscan](/wiki/economics/defi/token-registration/etherscan#who-signs-for-a-factory-deployed-token) covers that case.
...
There is no door. Neither Chainalysis nor TRM Labs publishes an address-label dispute process for an outside party, and the venue that screened you is the only party with a working one. This is covered under [liquidity and holders](/wiki/economics/defi/token-false-alarms/liquidity-and-holders#the-compliance-sibling).
