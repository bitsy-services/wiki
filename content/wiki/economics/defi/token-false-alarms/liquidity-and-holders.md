...
The practical consequence is that "locked" is a claim about your vendor relationships. [Locked liquidity](/wiki/economics/defi/par-token/locked-liquidity) is a property of the position; being *seen* as locked is a property of the locker's address being in a table.

## Concentrated liquidity has no top holders

"Top ten [LP](/wiki/economics/defi/markets/liquidity-pool) holders" is a well-defined quantity in Uniswap V2, where the pair contract *is* a fungible token, and undefined afterwards.
...
V3 replaces all of this with an [NFT](/wiki/economics/defi/blockchain/nft). Liquidity is an [ERC-721](/wiki/economics/defi/chains/ethereum/erc-721) minted by the `NonfungiblePositionManager`, fees accrue inside it, and there is no fungible balance to rank. `burn` reverts unless the position is already empty (`require(position.liquidity == 0 && position.tokensOwed0 == 0 && position.tokensOwed1 == 0, 'Not cleared')`), so live liquidity cannot be burned away at all — the position NFT has to be sent somewhere. Lockers adapted by escrowing the NFT: UNCX's V3 locker takes custody and revokes the owner's ability to move it between a start and an end date, while leaving fee collection available, which is the [fee-only ownership](/wiki/economics/defi/par-token/locked-liquidity#fee-only-ownership) shape.
...
mSOL retains mint authority because a liquid staking token must mint against deposits; without it the product does not work. USDC has the same two properties set on chain and scores 1, an exemption covered under [allowlists](/wiki/economics/defi/token-false-alarms/allowlists#membership-does-not-suppress-the-findings).
