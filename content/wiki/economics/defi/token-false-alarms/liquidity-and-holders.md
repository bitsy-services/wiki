...
The practical consequence is that "locked" is a claim about your vendor relationships. [Locked liquidity](/wiki/economics/defi/locked-liquidity) is a property of the position; being *seen* as locked is a property of the locker's address being in a table.

## Concentrated liquidity has no top holders

"Top ten [LP](/wiki/economics/defi/liquidity-pool) holders" is a well-defined quantity in Uniswap V2, where the pair contract *is* a fungible token, and undefined afterwards.
...
V3 replaces all of this with an [NFT](/wiki/economics/defi/nft). Liquidity is an [ERC-721](/wiki/economics/defi/ethereum/erc-721) minted by the `NonfungiblePositionManager`, fees accrue inside it, and there is no fungible balance to rank. `burn` reverts unless the position is already empty (`require(position.liquidity == 0 && position.tokensOwed0 == 0 && position.tokensOwed1 == 0, 'Not cleared')`), so live liquidity cannot be burned away at all — the position NFT has to be sent somewhere. Lockers adapted by escrowing the NFT: UNCX's V3 locker takes custody and revokes the owner's ability to move it between a start and an end date, while leaving fee collection available, which is the [fee-only ownership](/wiki/economics/defi/locked-liquidity#fee-only-ownership) shape.
