...
The token's verified source contains no restriction, only a call to an address the deployer controls and can repoint. Reading the token teaches nothing about whether it can be sold, and the [contract](/wiki/economics/defi/smart-contract) holding the answer is frequently unverified.
