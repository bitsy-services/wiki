...
ERC-1363 is an [ERC-20](/wiki/economics/finance/defi/ethereum/erc-20) extension that adds callback hooks to token transfers and approvals. It removes ERC-20's two-step problem: `approve` + `transferFrom` costs two transactions and leaves an allowance standing between them, while a plain `transfer` to a contract that was not expecting it leaves the tokens sitting there with nothing to trigger the deposit.
...
ERC-1363 covers the most ground of the three: both transfer and approval callbacks, the same magic-value confirmation ERC-721 uses, and no external registry of the kind ERC-777 depends on. ERC-777 was deprecated by OpenZeppelin in v5 due to persistent reentrancy vulnerabilities stemming from its pre-transfer hooks.

## Limitations

- **Receiver must opt in** -- contracts that don't implement `IERC1363Receiver` will cause `transferAndCall` to revert. Plain `transfer` still works, since the token is still an ERC-20, but the callback does not fire.
