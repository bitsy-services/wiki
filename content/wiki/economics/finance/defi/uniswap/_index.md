...
Getting a token to display correctly *in* the interface is a separate business from the protocol, and lives under token registration: [registering on Uniswap](/wiki/economics/finance/defi/token-registration/uniswap) covers where app.uniswap.org gets a token's logo, and why an unlisted token meets a warning screen.
