...
This makes the *net* storage cost of an application reflect the *net* data it leaves on-chain. Cleaning up state you no longer need is rewarded, not punished, which is the opposite of the implicit incentive on chains where freeing storage is at best a modest one-time refund. A common pattern in [Move](/wiki/economics/finance/defi/sui/sui-move) code is to delete objects when they are consumed precisely to recover this deposit.
