...
Trust Wallet's own help pages also say where its prices come from — "Trust Wallet sources pricing data from CoinMarketCap (CMC)" — and a [CoinMarketCap listing](/wiki/economics/finance/defi/token-registration/coinmarketcap) is one of the acceptance criteria below, so that submission comes first.
...
The folder name is the contract address in [Ethereum Improvement Proposal (EIP)](/wiki/economics/finance/defi/ethereum/eip) 55 checksummed form — the mixed-case spelling that encodes a checksum in the letter case. The repository's check compares the folder name against its own checksum computation and fails a lowercase folder. File names are compared as exact strings, so `logo.PNG` and `Info.JSON` fail too, and nothing but those two files may sit in the folder.
...
The 100 kB ceiling is enforced. The check divides the byte count by 1,024 with integer division and fails anything over 100, so a file must be under 103,424 bytes. [Making the icon](/wiki/economics/finance/defi/token-registration/icon#getting-under-100-kb) covers getting there.
...
What the files need, against the [token property schema](/wiki/economics/finance/defi/token-registration#the-token-property-schema):
