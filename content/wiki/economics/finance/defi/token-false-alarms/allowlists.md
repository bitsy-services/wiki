...
MetaMask's older on-repo allowlist, the `MetaMask/contract-metadata` repository, describes itself as effectively frozen and recommends the Ethereum Improvement Proposal (EIP) 747 wallet prompt instead, through a link to a registration guide that now 404s. It still merges submissions, slowly and at MetaMask's discretion, and an entry there counts as one of the upstream lists; [registering on MetaMask](/wiki/economics/finance/defi/token-registration/metamask) has the detail.
...
The submission side of that gate — how a logo actually reaches app.uniswap.org, and why a GeckoTerminal profile does not — is in [registering on Uniswap](/wiki/economics/finance/defi/token-registration/uniswap).
...
[Trust Wallet's](/wiki/economics/finance/defi/token-registration/trust-wallet) is the hardest published number in the entire landscape, and it is honest about what it is: the documentation says the fee model "was partly inspired by the concept of a Token Curated Registry", the fee is owed again for a logo change, and inclusions the team initiates itself are free. Its repository states outright that "brand new tokens are not accepted". [CoinGecko](/wiki/economics/finance/defi/token-registration/coingecko) is the mildest gate — tradability on a tracked exchange, with no depth or holder threshold published — and sells a Fast Pass that "guarantees faster processing, but does not guarantee that your request will be approved", with no refund.
