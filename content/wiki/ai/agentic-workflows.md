...
An agentic workflow is a [language model](/wiki/ai/llm) running in a loop with tools — planning, acting, observing the result, and deciding the next step — rather than answering in a single call. Every iteration re-reads the accumulated history, every tool result enters the window, and every wrong turn compounds into the next, so an agent spends roughly 4× the tokens of a single call and a multi-agent system roughly 15×. Getting value from an agent is mostly the discipline of *not* reaching for one until the problem needs it, then constraining it tightly when it does.

## Workflow versus agent

These two words are used interchangeably in most writing on the subject. They name different things: who owns the control flow.
...
Agents use roughly **4× the tokens of a single chat-style call, and multi-agent systems roughly 15×**. On agentic web-research evaluations, token volume alone explains about 80% of the performance variance — capability and cost rise together. An agent is justified only when the value of getting the task right is high enough to pay that multiple, and a single call cannot get it right. Run one on a task a fixed pipeline could have run and the multiple is paid anyway, for output the pipeline would have produced.

## Workflow patterns

Five structured patterns cover most of what an autonomous loop gets reached for, with fixed control flow and a bounded error surface:
...
A bare agent loop is simple: receive a goal, plan, call a tool, observe the result, decide whether the goal is met, and repeat or stop. What separates it from a chain of self-critiques is that **the environment supplies ground truth at every step**. A compiler error, a failing test, a search miss is feedback the model did not generate and cannot wish away, which is what lets an agent recover from its own mistakes. A loop without that grounding — one that only reasons about its own output — does not self-correct; it compounds.
...
The model reaches the rest of the system only through these contracts, so ambiguity in a tool's name, arguments, or description arrives as a wrong action rather than as a confused sentence.
...
An agent can check its own work against the environment, but only where verification is a step in the loop rather than an assumption about it.
