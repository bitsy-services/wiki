...
Do the arithmetic on what it costs. Two forward passes per weight, 124 million weights, so roughly 250 million forward passes — to compute *one* training step. GPT-2 was trained for hundreds of thousands of steps, which puts the whole run on the order of 10¹³ forward passes.

Backprop produces every one of those 124 million gradients for about the cost of two more forward passes. Not two per weight — two, total. That factor of tens of millions is not an optimization; it is the difference between training a [neural network](/wiki/ai/neural-network) and not, and the idea has survived unchanged since the 1980s while everything around it was replaced.
...
A product of a dozen small numbers is reliably a very small number. A sum that includes one path of derivative 1 — running straight down the skip path, touching no layer's weights at all — has no such tendency, though nothing makes it *impossible* for the remaining terms to cancel it out.

That structure is what [skip connections](/wiki/ai/neural-network/skip-connections) buy, and that page's caveat comes with it: in a model that also has [normalization](/wiki/ai/neural-network/normalization), taking the skips away does not in fact make gradients vanish. The sum-rather-than-product shape is real, and it is what gives the gradient a clean road home. The dramatic collapse it usually gets credited with preventing is a story about unnormalized networks, not this one.
...
Then the update itself: `w ← w − lr · ∂L/∂w`, where `lr` is the **learning rate**, a small number setting how far to move per step. Step downhill, a little. Adam, momentum, weight decay, learning-rate warmup — every optimizer anyone has built is a refinement of *how much to trust that one number*, not a replacement for it.
...
That's the standard gradient check, and how a hand-written backward pass gets proved wrong. It also costs two forward passes to verify a single weight out of 124 million — the cost argument above, run rather than asserted.
