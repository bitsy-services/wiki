...
One head cannot do all three, and the obvious explanation for that is wrong. It is *not* that the blend is unrecoverable — summing near-perpendicular directions is exactly how [the residual stream](/wiki/ai/llm/residual-stream) carries many things at once, and [superposition](/wiki/ai/neural-network/superposition) accounts for why that works.
...
The arithmetic works out to roughly what one 768-wide head would have cost, so the specialization is close to free: same budget, twelve questions instead of one.
