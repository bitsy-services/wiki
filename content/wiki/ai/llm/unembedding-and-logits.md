...
GPT-2 **ties** the unembedding to the [embedding](/wiki/ai/llm/embeddings) matrix — `W_U` is `W_E` transposed, the same 38.6M numbers doing duty at both edges. The vector you look a token up in is the vector you're scored against for predicting it.
