...
The transformer-specific words this subsection uses — and it uses only these. Third column: what the same thing is called elsewhere, so you can translate when you leave.

The general vocabulary every network shares — *layer*, *activations*, *bend*, *unit*, *gradient*, *feature* — is pinned in [the neural network glossary](/wiki/ai/neural-network/glossary) and not repeated here.
...
- **"Layer"** — ambiguous between a block and a sublayer inside it. Say *block*, *attention*, or *MLP*. (LayerNorm and [multi-layer perceptron](/wiki/ai/neural-network/multi-layer-perceptron) keep their names; those are fixed terms, not descriptions.) Note that [the neural network section](/wiki/ai/neural-network/glossary) does the opposite and uses *layer* freely — there is no ambiguity to avoid where there are no blocks.
