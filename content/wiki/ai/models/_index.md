...
A model builder is an organisation that trains [large language models](/wiki/ai/llm) and owns the resulting weights: the numbers a trained model consists of, which nobody writes by hand and which training alone sets. Training one at the leading edge means assembling three separately scarce things — a datacentre holding tens of thousands of accelerator chips, a corpus of text measured in tens of trillions of [tokens](/wiki/ai/llm/tokenization), and a team that has done it before. The capital requirement is what keeps the list short. This section covers eight organisations — five in the United States, two in China, one in France — and the model families they ship.

The first thing to know about any of them is whether you can download the weights. A **closed-weight** model exists only behind its builder's interface: requests go to the builder's hardware, and the weights never leave it. An **open-weight** model is published as a file anyone can run on their own machines. The split is not a proxy for quality — open-weight models from [DeepSeek](/wiki/ai/models/deepseek) and [Alibaba](/wiki/ai/models/qwen) have traded places with the closed frontier repeatedly — but it decides almost everything else about how a model can be used: whether it can process data that must not leave a building, whether a third party can fine-tune it, whether the builder can withdraw it, and whether anyone outside the lab can inspect what it actually does. "Open weights" is also not "open source": most open-weight licences in this section restrict something, and they do not all restrict the same thing — [Meta’s](/wiki/ai/models/meta) triggers on monthly active users, [Mistral’s](/wiki/ai/models/mistral) Modified MIT and [Alibaba’s](/wiki/ai/models/qwen) current flagship licence on revenue.

What these pages are careful about is the difference between what a lab **publishes** and what is **known**. [Meta](/wiki/ai/models/meta), [DeepSeek](/wiki/ai/models/deepseek), [Alibaba](/wiki/ai/models/qwen) and [Mistral](/wiki/ai/models/mistral) release parameter counts, training-token counts and architectural detail alongside their weights. [Anthropic](/wiki/ai/models/anthropic), [OpenAI](/wiki/ai/models/openai) and [Google](/wiki/ai/models/google) publish essentially none of it for their flagship models, and the confident figures circulating for those models come from leaks and repetition rather than disclosure. Where a page here states a model's size, a lab stated it; where a lab has not, the page says so instead of guessing.
...
- A **tier** is a size or capability rung within one generation, usually priced differently. Anthropic names its tiers with words (Haiku, Sonnet, Opus); OpenAI and Google use suffixes (`mini`, `nano`, Flash, Pro). [xAI](/wiki/ai/models/xai) numbers its generations and leaves it at that.
